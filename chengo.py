...
def parse_args():
    ...
    parser.add_argument('-b', '--bonus-field',dest='include_bonus_field',action='store_true',default=False,help='add one bonus field per card')
    parser.add_argument('-c', '--num-cards',dest='num_cards',type=int,default=1,help='number of cards to create (default: 1)')
    parser.add_argument('-s', '--card-size',dest='card_size',type=int,default=5,help='number of rows and columns per card (default: 5)')
    return parser.parse_args()

def load_words(filename):
    # Load words from a file. Every line is to be considered as one word.
...
def build_table(values, size, include_bonus_field=False):
    # Build a 2-dimensional table of the given size with unique, random values. If the table has more fields than values are available, the remaining fields will be empty.
    field_total = size ** 2
...
def collect_random_sublist(values, n):
    # Return unique, random elements from the values. Return `n` values or, if `n` is greater than the number of values, all values. The sample length must not exceed the number of available values.
    sample_length = min(n, len(values))
    return sample(values, sample_length)

def to_fields(values, n):
    # Create fields from the values, append infinite empty fields, and return the first `n` items.
